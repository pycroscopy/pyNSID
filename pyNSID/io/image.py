"""
:class:`~pyNSID.io.image.ImageTranslator` class that translates conventional 2D images to USID HDF5 files
...
class ImageTranslator(ArrayTranslator):
    ...
    def translate(self, image_path, h5_path=None, bin_factor=None, interp_func=Image.BICUBIC, normalize=False,
        ...
        dims = [Dimension('Y', 'Length', 'a.u.', np.arange(usize)),
                Dimension('X', 'Length', 'a.u.', np.arange(vsize))]

        # Need to transpose to for correct reshaping
        image = image.transpose()

        h5_path = super(ImageTranslator, self).translate(h5_path, 'Raw_Data', image,
                                                         'Intensity', 'a.u.', dims,

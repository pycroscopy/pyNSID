...
from .hdf_utils import get_all_main, read_h5py_dataset, check_if_main
...
class NSIDReader(Reader):
    ...
    def read(self, dataset=None):
        ...
        if dataset is None:
            return self.read_all_in_group(recursive=True)
        else:
            if isinstance(dataset, h5py.Dataset):
                return read_h5py_dataset(dataset)

    def read_all_in_file(self):
        list_of_main = get_all_main(self.h5_group.file, verbose=False)
        list_of_datasets = []
    ...
    def read_all_in_group(self, recursive=True):
        ...
        if recursive:
            ...
            for key in self.h5_group:
                if isinstance(self.h5_group[key], h5py.Dataset):
                    if check_if_main(self.h5_group[key]):
                        list_of_main.append(self.h5_group[key])
